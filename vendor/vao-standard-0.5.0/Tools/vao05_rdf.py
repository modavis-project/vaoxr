#!/usr/bin/env python3
"""Create a reversible annotated JSON-LD view of a VAO 0.5.0 manifest.
...
LOCAL_CONTEXT = vao_resources.schema_directory() / "vao-context-0.5.0.jsonld"
...
def project_jsonld(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not report["valid"]:
        raise ValueError(
            "Cannot project invalid VAO 0.5.0: " + "; ".join(report["errors"][:3])
        )
...
def project_offline_jsonld(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if manifest.get("@context") != [vao05.CONTEXT_URI]:
        raise ValueError(
